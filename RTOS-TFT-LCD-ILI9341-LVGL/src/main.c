...
void lv_termostato(void)
{
	...
	lv_obj_set_scrollbar_mode(steam_logo, LV_SCROLLBAR_MODE_OFF);
	...
	lv_label_set_text_fmt(labelSetValue, "%02d", 15);
	...
	lv_obj_align_to(labelFloorDecimal, labelFloor, LV_ALIGN_OUT_RIGHT_MID, 3, 13);
...
void lv_termostato_off(void)
{
	...
	lv_obj_t *labelBtnPower = lv_btn_create(lv_scr_act());
	lv_obj_add_event_cb(labelBtnPower, power_handler, LV_EVENT_ALL, NULL);
	lv_obj_align(labelBtnPower, LV_ALIGN_BOTTOM_LEFT, 9, -10);
	lv_obj_add_style(labelBtnPower, &style, 0);

	labelBtnPower = lv_label_create(labelBtnPower);
...
{
	calendar rtc_initial = {2022, 7, 25, 16, 16, 16, 1};
